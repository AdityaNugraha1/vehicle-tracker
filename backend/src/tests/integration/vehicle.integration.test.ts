// src/tests/integration/vehicle.integration.test.ts
...
// Setup: Buat Admin dan User, lalu login untuk mendapatkan token
beforeAll(async () => {
  // 1. Buat Admin User
...
describe('Vehicle & Trip API Integration Tests', () => {
  /**
   * Menguji Vehicle Management (Admin/Manager only)
   */
  ...
  /**
   * Menguji Vehicle Read (Any Authenticated User)
   */
  ...
  /**
   * Menguji Trip Management (Admin/Manager only)
   */
  describe('POST /api/vehicles/:vehicleId/trips/start & PATCH /api/vehicles/trips/:id/end', () => {
    ...
    it('should start a new trip for ADMIN (201)', async () => {
      ...
      // Verifikasi status kendaraan berubah
  ...
  /**
   * Menguji Vehicle Deletion (Admin only)
   */
  describe('DELETE /api/vehicles/:id', () => {
    ...
    it('should delete vehicle for ADMIN (200)', async () => {
      ...
      // Verifikasi kendaraan tidak ada lagi
      const vehicle = await prisma.vehicle.findUnique({
        where: { id: createdVehicleId },
      });
